...
The overseer has exactly three output surfaces: **message**, **signals**, and the **status window**. It does
not grow a fourth.
...
- Use sentence case and a terminal full stop; telegraphic prose belongs to the status window.
...
| Status window          | No. Telegraphic status only.                                                  |
...
The `overseer` window is summoned only by the system when a service starts work. No command or footer link
opens it. It is where trigger logs from every service land, centrally managed rather than fired ad hoc.
...
Every row is one of two kinds, and the distinction is what keeps the window honest:
...
- **Auto-summoned** when a row *appears* — a new condition, or any event.
- **Never re-summoned** by a `state` row revising itself, or by a replay to a joining tab. A row correcting
  itself is the window doing its job, not new work starting.
- **Appends live**, one row at a time.
...
| `discovery` | Show the tone's `lookingAround` copy, summon the status window, and stream discovery steps. |
...
status window, actionable results become signals, the message reflects activity, and actions enter the
...
A wipe that fails partway does not reload. The failed step stands in the status window and the error
...
| Status window                 | `packages/web/src/components/windows/OverseerWindow.tsx` |
