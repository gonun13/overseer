...
3. Explore the codebase at $2 using Read/Grep/Glob. This is real, open-ended
   exploration (not a single deterministic pass) — go as broad as you need
   to find the files/areas relevant to the request, existing conventions
   that should be followed, and any risks or ambiguities.
4. Only if local exploration leaves a genuine gap — e.g. the codebase
   ...
   WebSearch/WebFetch to check internet documentation. This is a fallback
   for real gaps, not a default step: prefer what's already in the codebase
   (comments, lockfile versions, README) first, and don't fetch pages
   unrelated to a concrete question raised by your exploration.
