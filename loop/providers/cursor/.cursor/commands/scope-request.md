...
Concrete arguments are bound by NAME at the end of this prompt. Copy values
by name (e.g. `planned_at`, `plan_file`). Never treat `$10` as `$1` plus `0`.

Named inputs (see Concrete arguments block):
- `record_file`, `research_file`, `decision_file` — scope I/O
- `id`, `workspace`, `slug`, `scoped_at`, `request_ref`, `research_ref`
- `plan_file`, `planned_at`, `scope_ref`, `workspace_dir`, `memory_file` — plan continue

## Step 1 — Read, don't explore

Read `record_file` (title, summary, description, kind, acceptance criteria) and `research_file`
...
Then call Write exactly once, targeting `decision_file`, with this exact
markdown document and nothing else — no code fences wrapping it, no
commentary before or after.

Copy these frontmatter values by NAME from Concrete arguments (verbatim):
`id`, `workspace`, `slug`, `scoped_at`, `request_ref`, `research_ref`.
`questions_asked` is the number of questions you put to the human in Step 2
(0-20 — never the confirmation in Step 3).
...
1. If `memory_file` exists, read it. Use Grep/Glob/Read on `workspace_dir`
   only when scope/research leave a concrete path unclear — not a full re-research.
...
5. Call Write exactly once on `plan_file` with the plan document below — no
   code fences, no commentary. Copy `id`, `workspace`, `slug`, `planned_at`,
   `request_ref`, `research_ref`, `scope_ref` by NAME from Concrete arguments.
   Set your own `impact` / `phase_count` integers (>= 1).
