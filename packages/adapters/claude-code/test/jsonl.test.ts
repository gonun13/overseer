...
describe("jsonl backfill", () => {
  ...
  it("leaves the CLI's own records out of the replayed transcript", async () => {
    // A session that opened with `/model haiku`. The caveat banner, the command
    // echo and its stdout are all `type: "user"`; replaying them would show the
    // operator saying things they never typed.
    configDir = await mkdtemp(path.join(tmpdir(), "claude-jsonl-"));
    const slugDir = path.join(
      configDir,
      "projects",
    ...
    const jsonl = [
      ...
      },
      {
        ...
        message: {
          role: "user",
          content: "<local-command-stdout>Set model to haiku</local-command-stdout>",
        },
      },
      {
        type: "user",
        ...
        message: {
          role: "user",
          content: [{ type: "text", text: "what is missing here?" }],
        },
      },
      {
        type: "assistant",
        ...
        message: {
          role: "assistant",
          content: [{ type: "text", text: "an approvals queue" }],
        },
      },
    ]
    ...
    const turns = await backfillHistory(configDir, projectDir, sessionId);
    assert.deepEqual(turns, [
      { id: "u1", kind: "user", text: "what is missing here?" },
      { id: "a1", kind: "agent", text: "an approvals queue" },
    ]);

    const listed = await listSessionsForProject(configDir, projectDir);
    assert.equal(listed[0]?.name, "what is missing here?");
  });
