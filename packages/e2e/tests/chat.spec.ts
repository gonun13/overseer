 ...
 * its own transcript and its own composer.
 *
...
test("opens a chat window per session, separate from the prompt", async ({
  ...
  // The bottom-left menu arms a session, so with none started there is nothing
  // for it to act on and it is not on the field.
  const menu = page.getByRole("button", { name: /context/i });
  ...
  // A window of its own — draggable and closable like every other, named for
  ...
  // still collapsed on its own line, offering commands and the overseer.
  await expect(
...
/**
 * The terminal is permanent furniture that changes size, not a window: it has
 * no tab and no close control, and Escape is what collapses it.
 */
test("the prompt terminal is a bare field, not a window", async ({ page }) => {
  ...
  const bar = page.getByRole("button", {
    name: /run a command, or message the overseer/i,
  });
  ...
  // The send hint belongs to session windows, where the surface is less familiar.
